...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add a simple click handler to the 'Explore Menus' button
    if (exploreBtn) {
        exploreBtn.addEventListener('click', (e) => {
            // Prevent the default anchor link navigation for this example
            e.preventDefault(); 
            console.log("Explore Menus button clicked!");
            // In a real site, you'd navigate here or load content
            // window.location.href = e.target.getAttribute('href'); 
            alert('Navigating to the Menus page...');
        });
    }

    // Add a simple click handler to the 'Start a Project' button
    if (startBtn) {
        startBtn.addEventListener('click', (e) => {
            e.preventDefault();
            console.log("Start a Project button clicked!");
            // Maybe open a contact form modal here
            alert('Opening Project Inquiry Form...');
        });

    }


    // Promo11 button dynamic action: navigate to contact or smooth scroll
    const promoBtn = document.querySelector('.background-section .image-button');
    if (promoBtn) {
        promoBtn.setAttribute('aria-label', 'Work With Us');
        promoBtn.addEventListener('click', (e) => {
            e.preventDefault();
            const img = promoBtn.querySelector('.button-img');
            if (img) {
